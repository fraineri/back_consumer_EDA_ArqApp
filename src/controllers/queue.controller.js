...
queue.CheckQueueNoSQS = () => {
  try {
    http
      .get(`${process.env.PRODUCER_URL}/GetItemFromQueue`, (resp) => {
        let data = "";
        ...
        resp.on("end", () => {
          ...
          console.log(
            `Read from queue = TestId: ${data.testId} - QuestionId: ${data.questionId}`
          );
          ...
          async function ProcessTestHeader(data) {
            try {
              ...
              if (!test.submited) {
                ...
                return console.log(
                  `Test ${test.testId} not submitted yet. Submited: ${test.submited} - Score: ${test.score}`
                );
              }

              if (test.score != -1) {
                return console.log(
                  `Test ${test.testId} already processed. Submited: ${test.submited} - Score: ${test.score}`
                );
              }
              ...
              await test.save();
              console.log(
                `Test ${test.testId} processed. Submited: ${test.submited} - Score: ${test.score}`
              );
            } catch (err) {
              console.error(err);
            }
...
queue.CheckQueueSQS = async () => {
  try {
    ...
    if (!sqs.IsQueueInitialized()) {
      return console.log("Queue not initialized yet");
    }
    const item = await sqs.PullFromQueue();
    if (!item.Messages) {
      return console.log("SQS Queue Empty");
    }

    const data = JSON.parse(JSON.parse(item.Messages[0].Body));

    await connection();
    console.log(`Conection Established`);
    console.log("data.testId", data.testId, typeof data.testId)
    let test = await TestModel.findById(data.testId);
